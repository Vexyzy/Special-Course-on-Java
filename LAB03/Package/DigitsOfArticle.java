...
public class DigitsOfArticle implements TextInformationImpl
{
    ...
    {   
        if(quality_of_page <= 0)
        {
            throw new Exception("Количество страниц не может быть <= 0");
        }
    ...
    public String toString()
    {
        ...
        stringBuilder.append("Название сборника статей: " + title + "\n");
        stringBuilder.append("Количество статей в сборнике: " + quality_of_page.length + "\n");
        ...
        for(int i = 0; i < quality_of_page.length; i++)
        {
            support_string = String.format("Количество страниц в статье %d: %d\n", i+1, quality_of_page[i]);
